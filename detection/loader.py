...
def load_raw_file(filepath: str) -> Dict[str, Any]:
    ...
    ch_splitbeam = ch_all[int(split_indices[0])]
    ...
    if "echo_range" in ds_Sv.coords:
        ranges = ds_Sv["echo_range"].sel(channel=ch_splitbeam).values.astype(float)
    ...
    sample_spacing_s = 2.0 * sample_spacing_m / sound_speed
